...
int main()
{
    ...
    // [단계 B] I2C 하드웨어 리셋 영역
    // ---------------------------------------------------------
    xil_printf("[DEBUG 3] Resetting AXI IIC Controller Registers...\n\r");
    ...
    xil_printf("[DEBUG 3] AXI IIC Controller Reset Complete!\n\r");
    ...
    // 루프 진입 전 가독성을 위해 터미널 줄을 하나 내려줍니다.
    xil_printf("\n\r");
    ...
    while (1) {
        ...
        // [수정됨] 모든 버튼 알림문에서 줄바꿈(\n\r)을 완전히 제거하여 한 줄 고정을 유지합니다.
        if (btn_press & 0x01) {
            display_mode = (display_mode + 1) % 3;
            xil_printf("\r[EVENT] Mode Changed to: %d                                     ", display_mode);
            usleep(500000); // 알림을 사용자가 읽을 수 있도록 0.5초 짧게 대기
        }
        if (btn_press & 0x02) {
        	offset_x = filtered_x;
        	offset_y = filtered_y;
        ...
        if (!is_paused) {
            ...
            // 6. [수정됨] UART 터미널 출력문
            // 문장 맨 앞에 '\r'을 배치하여 커서를 강제로 맨 앞으로 당기고,
            // 우측 끝에 넉넉한 공백을 추가하여 자릿수가 줄어들 때 잔상이 남지 않도록 완벽히 처리했습니다.
