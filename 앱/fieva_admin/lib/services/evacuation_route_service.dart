...
class EvacuationRouteService {
  ...
  static const _fireAvoidanceRadiusCad = 3500.0;
  ...
  EvacuationRouteResult _shortestEvacRoute(
    ...
    while (pending.isNotEmpty) {
      ...
      for (final nextId in current.neighbors) {
        ...
        if (_edgeIsUnsafe(current.point, next.point, firePoint)) continue;
        final candidate =
  ...
  bool _edgeIsUnsafe(Offset a, Offset b, Offset? firePoint) {
    if (firePoint == null) return false;
    return _distancePointToSegment(firePoint, a, b) < _fireAvoidanceRadiusCad;
  }
  ...
  double _distancePointToSegment(Offset point, Offset a, Offset b) {
    final dx = b.dx - a.dx;
    final dy = b.dy - a.dy;
    if (dx == 0 && dy == 0) return _offsetDistance(point, a);
    ...
    return _offsetDistance(
      point,
      Offset(a.dx + dx * clamped, a.dy + dy * clamped),
    );
  }
}
