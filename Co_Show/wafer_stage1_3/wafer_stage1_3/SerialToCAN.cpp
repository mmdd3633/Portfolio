...
// void Serial_CAN::begin(SoftwareSerial&  sw_serial, unsigned long baud)
// {
...
void Serial_CAN::begin(HardwareSerial &serial, unsigned long baud, int rxPin, int txPin) 
{
    // RX, TX 순서 주의!
    serial.begin(baud, SERIAL_8N1, rxPin, txPin);   // ESP32: 핀 지정
...
unsigned char Serial_CAN::recv(unsigned long *id, uchar *buf)
{
    if(canSerial->available())
    {
        ...
        while(1)
        {
          //Serial.print("recive ");  Serial.println((millis()-timer_s));
    ...
    // No complete CAN frame is available.
    return 0;
}

unsigned char Serial_CAN::cmdOk(const char *cmd)
{
...
{
    unsigned long baud[4] = {9600, 19200, 38400, 115200};
    int baudNow = 0;
...
unsigned char Serial_CAN::setMask(unsigned long *dta)
{
    ...
    for(int i=0; i<2; i++)
    {
        ...
        sprintf(str_tmp, "AT+M=[%d][%lu][", i, dta[2*i]);
        for(int i=0; i<8; i++)
        {
...
unsigned char Serial_CAN::setFilt(unsigned long *dta)
{
    ...
    for(int i=0; i<6; i++)
    {
        make8zerochar(8, __str, dta[1+2*i]);
        //Serial.println(__str);
        sprintf(str_tmp, "AT+F=[%d][%lu][", i, dta[2*i]);
        for(int i=0; i<8; i++)
        {
            str_tmp[12+i] = __str[i];
        }
