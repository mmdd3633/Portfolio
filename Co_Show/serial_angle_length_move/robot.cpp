...
int CAN_SERIAL_RX_PIN = 15;
int CAN_SERIAL_TX_PIN = 16;
uint32_t CAN_SERIAL_BAUD = 38400;
...
static const int Z_HOME_SENSOR_PIN = 9;
int detect = 0;
...
void system_init() {
  ...
  bus.begin(mySerial, CAN_SERIAL_BAUD, CAN_SERIAL_RX_PIN, CAN_SERIAL_TX_PIN);

  // Z-Axle Setup
  ledcAttach(PULSE_PIN, FREQ, RESOLUTION);
  pinMode(7, OUTPUT);
  pinMode(Z_HOME_SENSOR_PIN, INPUT_PULLUP);
}
...
void z_go_home(void) {
  ...
  while (1) {
    detect = digitalRead(Z_HOME_SENSOR_PIN);
    if (detect == 0) break;
    delay(1);
  }
