...
class FlutterBeaconUtils {
  ...
  static List<Map<String, Object>> beaconsToArray(List<Beacon> beacons) {
    ...
    for (Beacon beacon : beacons) {
      try {
        ...
        Log.w("BEACON", "Ignoring malformed beacon advertisement", error);
      }
    }
  ...
  private static Map<String, Object> beaconToMap(Beacon beacon) {
    ...
    if (id1 == null || id2 == null || id3 == null) {
      // UUID-only regions can also match advertisements without iBeacon IDs.
      return null;
    }

    Map<String, Object> map = new HashMap<>();
    map.put("proximityUUID", id1.toString().toUpperCase());
    map.put("major", id2.toInt());
    map.put("minor", id3.toInt());
